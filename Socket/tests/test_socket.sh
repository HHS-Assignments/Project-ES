#!/usr/bin/env bash
# test_socket.sh — End-to-end tests for the Pi-1 / Pi-2 socket pipeline.
#
# The script compiles the binaries (if not already built), starts Pi-2s and
# Pi-1s as background processes, sends synthetic HTTP POST requests that
# mimic the WMos D1 Mini, and verifies that Pi-2s prints the expected output.
#
# Exit codes:
#   0  All tests passed.
...
PI1_BIN="$SOCKET_DIR/Pi-1s-test"
PI2_BIN="$SOCKET_DIR/Pi-2s-test"
...
log "Compiling Pi-1s..."
gcc -O2 -o "$PI1_BIN" "$SOCKET_DIR/Pi-1s.c" "$SOCKET_DIR/cJSON.c" -lm || {
    echo "[FAIL] Compilation of Pi-1s failed"; exit 1
}

log "Compiling Pi-2s..."
gcc -O2 -o "$PI2_BIN" "$SOCKET_DIR/Pi-2s.c" "$SOCKET_DIR/cJSON.c" -lm || {
    echo "[FAIL] Compilation of Pi-2s failed"; exit 1
}

# ── Start servers ──────────────────────────────────────────────────────────

log "Starting Pi-2s on port $PI2_PORT..."
"$PI2_BIN" "$PI2_PORT" > /tmp/pi2_out.txt 2>&1 &
PI2_PID=$!

log "Starting Pi-1s on port $PI1_PORT forwarding to localhost:$PI2_PORT..."
...
# send_post <json_body>  — send an HTTP POST to Pi-1s.
# Returns the HTTP response body; exits with 0 even on curl errors so that
# application-level error paths (e.g. malformed JSON) can still be tested.
send_post() {
    local body="$1"
    curl -s --max-time 5 \
...
# wait_for_output <string> <timeout_seconds>
# Polls /tmp/pi2_out.txt until <string> appears or the timeout expires.
wait_for_output() {
    local needle="$1"
    local timeout="${2:-5}"
    local elapsed=0
    while [ "$elapsed" -lt "$timeout" ]; do
        grep -qF "$needle" /tmp/pi2_out.txt 2>/dev/null && return 0
        sleep 0.5
        elapsed=$((elapsed + 1))
    done
    return 1
...
if wait_for_output "Wmos" 5; then
...
if wait_for_output "Buttons, servos, temp" 5; then
    pass "Pi-2 printed Button category"
else
    fail "Pi-2 did not print Button category"
fi

if wait_for_output "1" 5; then
...
if wait_for_output "pressed" 5; then
