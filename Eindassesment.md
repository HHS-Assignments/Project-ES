# To-do lijst — Eindassessment

## Openstaande taken

### Lichtvoorziening
- [ ] PIR Motion sensor verplaatsen naar de lichtvoorziening (weg uit de luchtsluis) (Max)
- [ ] Mogelijkheid toevoegen om tot 2 kleuren tegelijk in te stellen, elk met een eigen helderheidswaarde (Kai)

### LED-configuratie
- [ ] Kleuren van de ledjes instelbaar maken via een menutje (Kai)

### Relaxhoek
- [ ] Afmaken wifi integratie Relaxhoek

### Balieconsole / Display
- [ ] Logische en duidelijke teksten schrijven voor op de display van de balieconsole (Tom)
- [x] Display verplaatsen naar buiten (Iedereen)

### Documentatie & Presentatie
- [x] Plattegrond maken voor bij het assessment (Max)

### Standaardfuncties
- [x] een .c file schrijven waar alle handige functies instaan zodat zo gelijk alles gebruikt kan worden (Max)
