...
int main(void)
{
  ...
  char GeenBeweging[] = "Geen Beweging gedetecteerd\r\n";
  ...
  while (1)
  {
	    uint8_t huh = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1);
	    if(huh == GPIO_PIN_SET){
	        HAL_UART_Transmit(&huart2, (uint8_t*)Beweging, strlen(Beweging), HAL_MAX_DELAY);
	        HAL_Delay(1000);
	    }
	    else{
	    	HAL_UART_Transmit(&huart2, (uint8_t*)GeenBeweging, strlen(GeenBeweging), HAL_MAX_DELAY);
	    	HAL_Delay(1000);
	    }
